...
export class ValidationPage extends BasePage {
    ...
    async validateRequiredFillName() {
        await this.safeVerifyToHaveText(this.REQUIRED_FILL_NAME, 'Nhập họ và tên');
    }

    async validateMinlengthEmail() {
        await this.safeVerifyToHaveText(this.MINLENGTH_EMAIL, 'Không nhập dưới 6 kí tự.');
    }

    async validateMaxLength245Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_245_CHARACTERS, 'Không nhập quá 245 kí tự.');
    }

    async validateMaxLength50Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_50_CHARACTERS, 'Không nhập quá 50 kí tự.');
    }

    async validateNoExistData() {
        await this.safeVerifyToHaveText(this.NO_EXIST_DATA, 'Không có dữ liệu');
    }

    async validateNameAlreadyExists() {
        await this.safeVerifyToHaveText(this.NAME_ALREADY_EXISTS, 'Tên đã tồn tại.');
    }

    async validateRequiredFillReason() {
        await this.safeVerifyToHaveText(this.REQUIRED_FILL_REASON, 'Nhập lý do');
    }

    async validateContractAlreadyApproved() {
        await this.safeVerifyToHaveText(this.CONTRACT_ALREADY_APPROVED, 'Đã có hợp đồng được duyệt');
    }

    async validateMaxLength255Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_255_CHARACTERS, 'Không nhập quá 255 kí tự.');
    }

    async validateMaxLength100Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_100_CHARACTERS, 'Không nhập quá 100 kí tự.');
    }

    async validateMaxLength20Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_20_CHARACTERS, 'Không nhập quá 20 kí tự.');
    }

    async validateMaxLength500Characters() {
        await this.safeVerifyToHaveText(this.MAX_LENGTH_500_CHARACTERS, 'Không nhập quá 500 kí tự.');
    }
}
